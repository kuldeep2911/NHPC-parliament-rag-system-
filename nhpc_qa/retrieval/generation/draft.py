...
def generate_draft(state, deps):
    ...
    if llm is None:
        ...
        #
        # `cfg` is the unified Settings object, so it already carries the LLM backend
        # fields and resolve_backends(). Before the restructure this had to build a
        # SECOND config object here (get_llm wanted a phase-2 Config, and Phase4Config
        # -- a different tree -- did not have resolve_backends(), which crashed with
        # AttributeError at runtime). One config removes that whole class of bug.
